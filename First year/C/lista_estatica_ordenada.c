...
void ordenar(lista *puntero) {
  for (int i = 0; i < (max - 1); i++) {
    for (int j = i + 1; j < max; j++) {
      if (((*puntero).contenido[i] > (*puntero).contenido[j]) &&
          ((*puntero).contenido[j] != 0) && ((*puntero).contenido[i] != 0)) {
        int aux = (*puntero).contenido[i];
        (*puntero).contenido[i] = (*puntero).contenido[j];
        (*puntero).contenido[j] = aux;
      }
    }
  }
}
