...
import com.seba.inventariado.model.Foto;
...
public class EditProductActivity extends AppCompatActivity {
    ...
    private final HashMap<String, Long> nombreImagenesServidor = new HashMap<>();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        toolbar.setTitle(R.string.edit_product_title);
        ...
        descripcionProducto.setOnClickListener(v -> {
            Intent intent = new Intent(EditProductActivity.this, DescriptionInputActivity.class);
        ...
        alarma = findViewById(R.id.alarma_new_product);
        alarma.setOnClickListener(click -> {
            ...
            minLvlOptionSelected.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    if (position == 0) {
                        minLvlOption = "0";
                        minLvlEnabled = false;
                        alarma.setImageDrawable(AppCompatResources.getDrawable(EditProductActivity.this, R.drawable.ic_bell_disabled));
                    }
                }
        ...
        addPhotos.setOnClickListener(v ->
                FishBun.with(EditProductActivity.this)
                        ...
                        .setHomeAsUpIndicatorDrawable(ContextCompat.getDrawable(EditProductActivity.this, R.drawable.ic_arrow_back_black_24dp))
                        .setDoneButtonDrawable(ContextCompat.getDrawable(EditProductActivity.this, R.drawable.ic_check_black_24dp))
        ...
        botonGuardarProducto.setOnClickListener((v) -> {
            if (!StringUtils.isBlank(nombreProducto.getText()) && !StringUtils.isBlank(cantidadProducto.getText().toString()) && !StringUtils.isBlank(precioProducto.getText().toString())) {
                ...
                Toast.makeText(this, R.string.invalid_product, Toast.LENGTH_LONG).show();
            }
        });
            ...
            if (resultCode == RESULT_OK) {
                for (Parcelable foto : data.getParcelableArrayListExtra(INTENT_PATH)) {
                    ...
                    try {
                        ...
                        String nombreImagen = imagen.getUriData(this).first;
                        Long sizeImagen = imagen.getUriData(this).second;

                        if (!listaImagenes.contains(imagen) && !nombreImagenesServidor.containsKey(nombreImagen) && listaImagenes.size() < 11) {
                            nombreImagenesServidor.put(nombreImagen, sizeImagen);
                ...
                PreviewImageAdapter pv = new PreviewImageAdapter(this, listaImagenes, imageSlider, recListItems, switcher, nombreImagenesServidor);
                recListItems.setAdapter(pv);
            } else if (resultCode == RESULT_CANCELED) {
                Toast.makeText(EditProductActivity.this, "You haven't picked Image", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(EditProductActivity.this, "Something went wrong", Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private void getProduct(String productID) {
        ...
        Call<Producto> LoginResponseCall = getTags.searchProduct(UUID.fromString(productID));
        LoginResponseCall.enqueue(new Callback<Producto>() {
            ...
            public void onResponse(@NotNull Call<Producto> call, @NotNull Response<Producto> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    nombreProducto.setText(StringUtils.defaultIfBlank(product.getNombre(), ""));
                    descripcionProducto.setText(StringUtils.defaultIfBlank(product.getDescripcion(), null));
                    cantidadProducto.setText(StringUtils.defaultIfBlank(product.getCantidad(), "0"));
                    precioProducto.setText(StringUtils.defaultIfBlank(product.getPrecio(), "0"));
                    totalValue.setText(new BigInteger(StringUtils.defaultIfBlank(product.getPrecio(), "0")).multiply(new BigInteger(StringUtils.defaultIfBlank(product.getCantidad(), "0"))).toString().replaceAll("\\B(?=(\\d{3})+(?!\\d))", "."));
                    minLvl = product.getMinLvl();
                    minLvlOption = product.getMinLvlOption();
                    minLvlEnabled = product.isMinLvlEnabled();

                    if (minLvlEnabled)
                        alarma.setImageDrawable(AppCompatResources.getDrawable(EditProductActivity.this, R.drawable.ic_bell_enabled));

                    if (product.getTags() != null && !product.getTags().isEmpty()) {
                        for (Tag tag : product.getTags()) {
                            completionView.addObjectSync(tag);
                        }
                    }

                    if (product.getAllFotosName() != null) {
                        nombreImagenesServidor.putAll(product.getAllFotosName());
                    }

                    if (product.getFotos() != null && !product.getFotos().isEmpty()) {
                        SessionManager session = new SessionManager(EditProductActivity.this);
                        Users users = session.getUserDetails();

                        for (Foto nombreFoto : product.getFotos()) {
                            TipoImagen fotoURL = new TipoImagen(getResources().getString(R.string.productPhotosURL, users.getUserID(), product.getId().toString(), nombreFoto.getNombreImagen()));

                            if (!listaImagenes.contains(fotoURL) && listaImagenes.size() < 11) {
                                listaImagenes.add(fotoURL);
                                if (switcher.getDisplayedChild() == 0) {
                                    switcher.setDisplayedChild(1);
                                }
                            } else if (listaImagenes.size() > 10) {
                        ...
                        PreviewImageAdapter pv = new PreviewImageAdapter(EditProductActivity.this, listaImagenes, imageSlider, recListItems, switcher, nombreImagenesServidor);
                        recListItems.setAdapter(pv);
                    }
    ...
    private void saveProduct() {
        ...
        try {
            ...
            productoAGuardar.put("fileNames", (nombreImagenesServidor.size() > 0) ? nombreImagenesServidor.toString() : null);
        ...
        Call<Producto> LoginResponseCall = saveProduct.editProduct(productID, requestBody, list); //builder.build()
        LoginResponseCall.enqueue(new Callback<Producto>() {
            @Override
            public void onResponse(@NotNull Call<Producto> call, @NotNull Response<Producto> response) {
                finish();
            ...
            @Override
            public void onFailure(@NotNull Call<Producto> call, @NotNull Throwable t) {
                Toast.makeText(EditProductActivity.this, "An error ocurred, please try again, or report it.", Toast.LENGTH_LONG).show();
                finish();
                /*new AlertDialog.Builder(EditProductActivity.this)
                        .setTitle("Error")
                        .setMessage("Internet not available, check your internet connection and try again.")
    ...
    private void getTags() {
        ...
        session = new SessionManager(EditProductActivity.this);
        ...
        LoginResponseCall.enqueue(new Callback<List<Tag>>() {
            ...
            public void onResponse(@NotNull Call<List<Tag>> call, @NotNull Response<List<Tag>> response) {
                ...
                ArrayAdapter<Tag> adapter = new ArrayAdapter<>(EditProductActivity.this, android.R.layout.simple_spinner_dropdown_item, allTags);
